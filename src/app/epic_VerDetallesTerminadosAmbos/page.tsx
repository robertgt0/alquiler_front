...
function DetailsPage() {
  ...
  const id = sp.get('id') ?? '';
  // 🔹 CAMBIO: Ahora usa 'role' del URL, no 'from'
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        // 🔹 ELIMINADO: Ya no usa localStorage, va directo al backend
  ...
  // 🔹 CAMBIO: Ahora detecta correctamente según el rol
  ...
  // 🔹 CAMBIO: Ahora busca en price, cost, y costo
  const costoNum = j.price ?? j.cost ?? j.costo ?? 0;
  const costo = costoNum > 0 ? `${costoNum} Bs` : '—';
  ...
  return (
    <main style={{ padding: 24, maxWidth: 680, margin: '0 auto' }}>
      ...
      <section style={styles.section}>
        ...
        <Row label="Servicio:" value={job.service ?? '—'} />
      ...
      <div style={styles.footer}>
        ...
        <button style={styles.btnBlue} onClick={() => alert('Abrir flujo de calificación')}>
          {/* 🔹 CAMBIO: El texto del botón cambia según el rol */}
