...
export default function GestionCitas() {
  ...
  const confirmarCancelacion = async () => {
    try {
      ...
      setShowSuccessModal(true); // Mostrar modal de éxito aquí
    } catch (err) {
      console.error(err);
            ...
            <div className="flex items-center justify-center min-h-full p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  {selectedDayCitas && selectedDayCitas.length > 0 ? (
                    <ul className="divide-y divide-gray-200">
                      {selectedDayCitas.map((cita) => (
                        <li key={cita._id} className="py-2">
                          <p className="text-sm font-semibold text-blue-700">
                            {cita.servicioId?.nombre || "Servicio sin nombre"}
                          </p>
                          ...
                          <p className="text-sm text-gray-500">
                            UBICACIÓN: {cita.ubicacion?.direccion || "Sin dirección registrada"}
                          </p>
                        </li>
                      ))}
                    </ul>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4">
            ...
            >
              <Dialog.Panel className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 text-center">
                ...
                <p className="text-gray-700 text-sm mb-4">
                  Notificaciones enviadas. Las citas de este día fueron canceladas por motivos de fuerza mayor.
                </p>
